...
A wrapper for the GAP package ferret 1.0.14.
"""
...
Load the GAP package ferret into GAP. Use `banner` to control
...
function load(; banner::Bool=true, only_needed::Bool=true)
    res = GAP.Globals.LoadPackage(GapObj("ferret"), banner; OnlyNeeded=only_needed)
    if res != true
